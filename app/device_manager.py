...
class DeviceManager(
    ...
    async def execute(self, owner: str, device_identifier: str, data: dict[str, Any]) -> bool:
        ...
        # Checkout receipts use the structured payload. Route them through the
        # same primary printer binding as the verified preview path.
        if (
            device_identifier == "printer_rp_12n"
            and isinstance(data.get("receipt"), dict)
            and isinstance(data["receipt"].get("structured"), dict)
            and "printer_main" in self.devices
        ):
